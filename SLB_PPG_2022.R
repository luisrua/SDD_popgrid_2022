# POPULATION GRID Solomon Islands - SLB - 2022 UPDATE
...
census_year <- 2009
current_year <- 2022
pop_2009 <- 515883 # calculated from latest census data
prjpop_2022 <- 744407  # From SDD .STAT population projections

# POULATION SPATIAL DISTRIBUTION INPUT
# HH locations and population from 2009 PHC HH listing (2019 PHC has not been released yet)
pts <- st_read(paste0(dd,"/layers/HH_GPS_point_rmahs4326Cpop.shp"))
pts$pop <- pts$RMAVS2
pop_2009 <- sum(pts$RMAVS2)

# check if there is pgr defined in census report
popGR <- (prjpop_2022 - pop_2009)/((current_year - census_year)*pop_2009)
popGR
...
rastpop2009_100m <- rasterize(pts,rast100m,'pop',fun=sum)
rastpop2009_100m
totpop2009_count <- cellStats(rastpop2009_100m, 'sum')
totpop2009_count
...
pop_dif <- totpop2009_count*popGR * (as.numeric(current_year)- as.numeric(census_year))
pop_dif
...
pts2009$totpop2022 <- (pts2009$pop + (pts2009$pop * popGR * (as.numeric(current_year)- as.numeric(census_year))))
pts2009

pts_totpop2022 <- sum(pts2009$totpop2022,na.rm = T)
pts_totpop2009 <- sum(pts2009$pop,na.rm = T)

pop_evolution <- pts_totpop2022 - pts_totpop2009
pop_evolution

# round and check to see if it works
round_preserve_sum(pts2009$totpop2022)

pts_totpop2022_rps<- sum(round_preserve_sum(pts2009$totpop2022))
pts_totpop2022_rps
dif <- pts_totpop2022_rps - pts_totpop2022
dif

# create the rounded field and add it into the att table
pts2009$totpop2022rps <- round_preserve_sum(pts2009$totpop2022)
pts2009

sum(pts2009$totpop2022rps)
# st_write(pts2020,"layers/pts2020.shp",delete_layer=TRUE)

rastpop2022rps_100m<- rasterize(pts2009,rast100m,'totpop2022rps',fun=sum)
rastpop2022rps_100m
