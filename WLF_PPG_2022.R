...
census_year <- 2018
current_year <- 2022
pop_2018 <- 11558  # calculated from latest census data
prjpop_2022 <-  11302  # From SDD .STAT population projections

# check if there is pgr defined in census report
popGR <- (prjpop_2022 - pop_2018)/((current_year - census_year)*pop_2018)
popGR

# POULATION SPATIAL DISTRIBUTION INPUT
# HH locations and population from 2020 PHC
pts <- st_read(paste0(dd,"/layers/pts2018.gpkg"))
pts$pop <- pts$tpop2018
...
rastpop2018_100m <- rasterize(pts,rast100m,'pop',fun=sum)
rastpop2018_100m
...
pts2018$totpop2022 <- (pts2018$pop + (pts2018$pop * popGR * (as.numeric(current_year)- as.numeric(census_year))))
pts2018

pts_totpop2022 <- sum(pts2018$totpop2022,na.rm = T)
pts_totpop2018 <- sum(pts2018$pop,na.rm = T)

pop_evolution <- pts_totpop2022 - pts_totpop2018
pop_evolution

# round and check to see if it works
round_preserve_sum(pts2018$totpop2022)

pts_totpop2022_rps<- sum(round_preserve_sum(pts2018$totpop2022))
pts_totpop2022_rps
dif <- pts_totpop2022_rps - pts_totpop2022
dif

# create the rounded field and add it into the att table
pts2018$totpop2022rps <- round_preserve_sum(pts2018$totpop2022)
pts2018

sum(pts2018$totpop2022rps)
# st_write(pts2020,"layers/pts2020.shp",delete_layer=TRUE)

rastpop2022rps_100m<- rasterize(pts2018,rast100m,'totpop2022rps',fun=sum)
rastpop2022rps_100m
